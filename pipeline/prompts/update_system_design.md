...
3. **动态多维度拆解**：文档必须从多个维度进行深度剖析。这些拆分的维度**不是固定的**，你需要根据项目类型和具体需求**自行分析和决定**。
...
### 第一步：读取现有文档
用 Read 工具读取 `oh-gen-doc/` 目录下现有的设计文档（如 `oh-gen-doc/system_design.yaml` 或其他按维度拆分的 `.yaml` 文件）。如果不存在则准备从头创建。
...
### 第五步：写回文档
用 Write 或 Edit 工具将合并后的完整内容写回 `oh-gen-doc/系统设计.yaml`（或者根据你拆分的维度，写入 `oh-gen-doc/` 下的多个中命名 `.yaml` 文件中，如 `oh-gen-doc/接口定义.yaml`, `oh-gen-doc/页面元素.yaml` 等，保持结构清晰）。
