...
### 第一步：读取现有文档
用 Read 工具读取 `docs/` 目录下现有的设计文档（如 `docs/system_design.yaml` 或其他按维度拆分的 `.yaml` 文件）。如果不存在则准备从头创建。
...
### 第五步：写回文档
用 Write 或 Edit 工具将合并后的完整内容写回 `docs/系统设计.yaml`（或者根据你拆分的维度，写入 `docs/` 下的多个中命名 `.yaml` 文件中，如 `docs/接口定义.yaml`, `docs/页面元素.yaml` 等，保持结构清晰）。
